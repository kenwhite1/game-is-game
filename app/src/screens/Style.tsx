      ...
      {slot === 'color' && (() => {
        ...
        return (
          <div className="recolor-row">
            <div className="rr-head">🎨 Перекрасить тело <span className="sub">{RECOLOR_COST[rarity]} G · сброс бесплатно</span></div>
            <div className="rr-sw-row">
              {Array.from({ length: RECOLOR_STEPS }, (_, i) => i * 30).map(h => (
                <button key={h} className={`rr-sw ${curHue === h ? 'on' : ''}`} onClick={() => void recolor(profile.color, h)}
                  style={{ background: bodyHex, filter: hueFilter(h) }} aria-label={h === 0 ? 'оригинал' : `оттенок ${h}°`} />
              ))}
            </div>
          </div>
        )
